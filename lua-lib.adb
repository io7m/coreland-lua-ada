...
package body lua.lib is
  ...
  function new_metatable (ls: lua.state_ptr_t; name: string) return boolean is
  begin
  ...
  procedure open_library (ls: lua.state_ptr_t; name: string; funcs: reg_array;
    num_up: integer) is
  begin
    if name /= "" then
      lua.push_string (ls, name);
  ...
  procedure lua_openbase (ls: lua.state_ptr_t);
  pragma import (c, lua_openbase, "luaopen_base");

  procedure open_base (ls: lua.state_ptr_t) is
  begin
    lua_openbase (ls);
  end open_base;

  procedure lua_opentable (ls: lua.state_ptr_t);
  pragma import (c, lua_opentable, "luaopen_table");

  procedure open_table (ls: lua.state_ptr_t) is
  begin
    lua_opentable (ls);
  end open_table;

  procedure lua_openstring (ls: lua.state_ptr_t);
  pragma import (c, lua_openstring, "luaopen_string");

  procedure open_string (ls: lua.state_ptr_t) is
  begin
    lua_openstring (ls);
  end open_string;

  procedure lua_openio (ls: lua.state_ptr_t);
  pragma import (c, lua_openio, "luaopen_io");

  procedure open_io (ls: lua.state_ptr_t) is
  begin
    lua_openio (ls);
  end open_io;

  procedure lua_openmath (ls: lua.state_ptr_t);
  pragma import (c, lua_openmath, "luaopen_math");

  procedure open_math (ls: lua.state_ptr_t) is
  begin
    lua_openmath (ls);
  end open_math;

  procedure lua_opendebug (ls: lua.state_ptr_t);
  pragma import (c, lua_opendebug, "luaopen_debug");

  procedure open_debug (ls: lua.state_ptr_t) is
  begin
    lua_opendebug (ls);
  end open_debug;

  procedure lual_openlibs (ls: lua.state_ptr_t);
  pragma import (c, lual_openlibs, "luaL_openlibs");

  procedure open_libs (ls: lua.state_ptr_t) is
  begin
    lual_openlibs (ls);
  end open_libs;
