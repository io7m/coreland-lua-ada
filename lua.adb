...
package body lua is
  ...
  function lua_checkstack (state : state_t; extra : int_t) return int_t;
  pragma import (c, lua_checkstack, "lua_checkstack");

  procedure lua_pushvalue (state : state_t; index : int_t);
  pragma import (c, lua_pushvalue, "lua_pushvalue");

  procedure lua_remove (state : state_t; index : int_t);
  pragma import (c, lua_remove, "lua_remove");

  procedure lua_insert (state : state_t; index : int_t);
  pragma import (c, lua_insert, "lua_insert");

  procedure lua_replace (state : state_t; index : int_t);
  ...
  function lua_typename (state : state_t; index : int_t) return ics.chars_ptr;
  pragma import (c, lua_typename, "lua_typename");

  function lua_type (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_type, "lua_type");

  function lua_isnumber (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_isnumber, "lua_isnumber");

  function lua_isstring (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_isstring, "lua_isstring");

  function lua_iscfunction (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_iscfunction, "lua_iscfunction");

  function lua_isuserdata (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_isuserdata, "lua_isuserdata");

  function lua_equal (state : state_t; index1, index2 : int_t) return int_t;
  pragma import (c, lua_equal, "lua_equal");

  function lua_rawequal (state : state_t; index1, index2 : int_t) return int_t;
  pragma import (c, lua_rawequal, "lua_rawequal");

  function lua_lessthan (state : state_t; index1, index2 : int_t) return int_t;
  pragma import (c, lua_lessthan, "lua_lessthan");

  function lua_toboolean (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_toboolean, "lua_toboolean");

  function lua_tonumber (state : state_t; index : int_t) return number_t;
  pragma import (c, lua_tonumber, "lua_tonumber");

  function luext_tostring (state : state_t; index : int_t) return ics.chars_ptr;
  pragma import (c, luext_tostring, "luext_tostring");

  function lua_objlen (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_objlen, "lua_objlen");

  function lua_tocfunction (state : state_t; index : int_t) return user_func_t;
  ...
  procedure lua_pushboolean (state : state_t; b : int_t);
  ...
  procedure lua_pushstring (state : state_t; s : ics.chars_ptr);
  pragma import (c, lua_pushstring, "lua_pushstring");

  procedure lua_pushlstring (state : state_t; s : system.address; a : integer);
  ...
  procedure lua_pushcclosure (state : in state_t; fn : in user_func_t; num : in int_t);
  ...
  function lua_next (state : state_t; index : int_t) return int_t;
  ...
  function lua_getmetatable (state : state_t; index : int_t) return int_t;
  pragma import (c, lua_getmetatable, "lua_getmetatable");

  function lua_setmetatable (state : state_t; index : int_t) return int_t;
  ...
  procedure lua_getfenv (state : state_t; index : int_t);
  pragma import (c, lua_getfenv, "lua_getfenv");

  function lua_setfenv (state : state_t; index : int_t) return int_t;
  ...
  procedure lua_call (state : in state_t; num_args : in int_t; num_results : in int_t);
  pragma import (c, lua_call, "lua_call");

  function lua_pcall (state : in state_t; num_args : in int_t; num_results : in int_t;
  ...
  procedure lua_createtable (state : state_t; num_arr : int_t; num_rec : int_t);
  pragma import (c, lua_createtable, "lua_createtable");

  procedure lua_settable (state : state_t; index : int_t);
  pragma import (c, lua_settable, "lua_settable");

  procedure lua_gettable (state : state_t; index : int_t);
  pragma import (c, lua_gettable, "lua_gettable");

  procedure lua_rawget (state : state_t; index : int_t);
  pragma import (c, lua_rawget, "lua_rawget");

  procedure lua_rawset (state : state_t; index : int_t);
  pragma import (c, lua_rawset, "lua_rawset");

  procedure lua_rawgeti (state : state_t; index, element : int_t);
  pragma import (c, lua_rawgeti, "lua_rawgeti");

  procedure lua_rawseti (state : state_t; index, element : int_t);
  ...
  function lua_atpanic (state : state_t; func : user_func_t) return user_func_t;
  ...
  procedure lua_sethook (state : state_t; func : hook_t; amask, count : int_t);
  ...
  procedure lua_setfield (state : state_t; index : int_t; key : ics.chars_ptr);
  ...
  procedure lua_getfield (state : state_t; index : int_t; key : ics.chars_ptr);
  ...
  type buffer_func is
    access function (state : state_t; data : access buffer_ctx; size : access ic.size_t)
      return ics.chars_ptr;
  pragma convention (c, buffer_func);

  function lua_load_buffer (state : state_t;
    func : buffer_func;
  ...
  function load_buffer (state : state_t; str : ics.chars_ptr; size : ic.size_t; name : string) return error_t is
  ...
  function load_buffer (state : state_t; str : string; size : natural; name : string) return error_t is
  ...
  function load_buffer (state : state_t; str : su.unbounded_string; name : string) return error_t is
  begin
  ...
  function load_string (state : state_t; str : string) return error_t is
  begin
  ...
  function exec_string (state : state_t; str : string) return error_t is
    ec : error_t;
  begin
  ...
  procedure set_hook (state : state_t; func : hook_t; amask : mask_t; count : integer) is
  begin
  ...
  function at_panic (state : state_t; panic_function : user_func_t) return user_func_t is
  begin
  ...
  function check_stack(state : state_t; size : integer) return integer is
  begin
  ...
  function is_user_data (state : state_t; index : integer) return boolean is
  begin
  ...
  function to_cfunction (state : state_t; index : integer) return user_func_t is
  begin
    return lua_tocfunction (state, int_t (index));
  end to_cfunction;

  function is_equal (state : state_t; index1, index2 : integer) return boolean is
  begin
    return lua_equal (state, int_t (index1), int_t (index2)) /= 0;
  end is_equal;

  function is_raw_equal (state : state_t; index1, index2 : integer) return boolean is
  begin
    return lua_rawequal (state, int_t (index1), int_t (index2)) /= 0;
  end is_raw_equal;

  function is_less_than (state : state_t; index1, index2 : integer) return boolean is
  begin
    return lua_lessthan (state, int_t (index1), int_t (index2)) /= 0;
  end is_less_than;

  function objlen (state : state_t; index : integer) return int_t is
  begin
    return lua_objlen (state, int_t (index));
  end objlen;

  function strlen (state : state_t; index : integer) return int_t is
  begin
    return lua_objlen (state, int_t (index));
  end strlen;
  ...
  procedure push_string (state : state_t; str : su.unbounded_string) is
    ch_array : aliased ic.char_array := ic.to_c (su.to_string (str));
  begin
    lua_pushstring (state, ics.to_chars_ptr (ch_array'unchecked_access));
  end push_string;

  procedure push_string (state : state_t; address : system.address; amount : positive) is
  begin
    lua_pushlstring (state, address, amount);
  end push_string;

  procedure push_user_closure (state : state_t; func : user_func_t; num_params : integer) is
  begin
    lua_pushcclosure (state, func, int_t (num_params));
  end push_user_closure;

  procedure push_user_function (state : state_t; func : user_func_t) is
  begin
  ...
  procedure raw_get_int (state : state_t; index : integer; element : integer) is
  begin
    lua_rawgeti (state, int_t (index), int_t (element));
  end raw_get_int;

  procedure get_field (state : state_t; index : integer; str : ics.chars_ptr) is
  begin
    lua_getfield (state, int_t (index), str);
  end get_field;

  procedure get_field (state : state_t; index : integer; str : string) is
    ch_array : aliased ic.char_array := ic.to_c (str);
  begin
    lua_getfield (state, int_t (index), ics.to_chars_ptr (ch_array'unchecked_access));
  end get_field;

  procedure get_field (state : state_t; index : integer; str : su.unbounded_string) is
    ch_array : aliased ic.char_array := ic.to_c (su.to_string (str));
  begin
    lua_getfield (state, int_t (index), ics.to_chars_ptr (ch_array'unchecked_access));
  end get_field;
 
  procedure get_global (state : state_t; str : ics.chars_ptr) is
  begin
    lua_getfield (state, globalsindex, str);
  end get_global;

  procedure get_global (state : state_t; str : string) is
    ch_array : aliased ic.char_array := ic.to_c (str);
  begin
    lua_getfield (state, globalsindex, ics.to_chars_ptr (ch_array'unchecked_access));
  end get_global;

  procedure get_global (state : state_t; str : su.unbounded_string) is
    ch_array : aliased ic.char_array := ic.to_c (su.to_string (str));
  begin
    lua_getfield (state, globalsindex, ics.to_chars_ptr (ch_array'unchecked_access));
  end get_global;

  procedure get_fenv (state : state_t; index : integer) is
  begin
  ...
  procedure raw_set_int (state : state_t; index : integer; element : integer) is
  begin
    lua_rawseti (state, int_t (index), int_t (element));
  end raw_set_int;

  procedure set_field (state : state_t; index : integer; str : ics.chars_ptr) is
  begin
    lua_setfield (state, int_t (index), str);
  end set_field;

  procedure set_field (state : state_t; index : integer; str : string) is
    ch_array : aliased ic.char_array := ic.to_c (str);
  begin
    lua_setfield (state, int_t (index), ics.to_chars_ptr (ch_array'unchecked_access));
  end set_field;

  procedure set_field (state : state_t; index : integer; str : su.unbounded_string) is
    ch_array : aliased ic.char_array := ic.to_c (su.to_string (str));
  begin
    lua_setfield (state, int_t (index), ics.to_chars_ptr (ch_array'unchecked_access));
  end set_field;

  procedure set_global (state : state_t; str : ics.chars_ptr) is
  begin
    lua_setfield (state, globalsindex, str);
  end set_global;

  procedure set_global (state : state_t; str : string) is
    ch_array : aliased ic.char_array := ic.to_c (str);
  begin
    lua_setfield (state, globalsindex, ics.to_chars_ptr (ch_array'unchecked_access));
  end set_global;

  procedure set_global (state : state_t; str : su.unbounded_string) is
    ch_array : aliased ic.char_array := ic.to_c (su.to_string (str));
  begin
    lua_setfield (state, globalsindex, ics.to_chars_ptr (ch_array'unchecked_access));
  end set_global;
  ...
  function lua_resume (state : state_t; narg : int_t) return int_t;
  ...
  function resume (state : state_t; num_args : integer) return error_msg_t is
    error : error_t;
  ...
  procedure register (state : state_t; func_name : string; func_ptr : user_func_t) is
  begin
  ...
  procedure call (state : state_t; num_arguments, num_results : integer) is
  begin
    lua_call (state, int_t (num_arguments), int_t (num_results));
  end call;

  function pcall (state : state_t; num_arguments, num_results, error_func : integer)
    return error_t is
  begin
  ...
  procedure unreference (state : state_t; ref : obj_ref_t) is
  begin
    unreference (state, registry_index, ref);
  end unreference;

  procedure dereference (state : state_t; ref : obj_ref_t) is
  begin
